...
const TRANSITIONS = [
  ...
  { id: "pixelize", label: "Пикселизация" },
];
...
export function ConcatTab() {
  ...
  return html`
    ...
    <div class="mt-5 flex flex-wrap items-end gap-3">
      <label class="text-sm">
        <span class="mb-1 block text-neutral-400">Переход</span>
        <select value=${transition} onChange=${e => setTransition(e.target.value)}
          class="rounded-lg border border-neutral-700 bg-neutral-900 px-3 py-2 outline-none focus:border-indigo-500">
          ${TRANSITIONS.map(t => html`<option key=${t.id} value=${t.id}>${t.label}</option>`)}
        </select>
      </label>
      <label class="text-sm">
